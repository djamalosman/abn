...
class Generatesp2 extends CI_Model {
 ...
 public function gensp() {
	$bln = date('n');
        ...
        foreach ($hasilselectdebitur as $debitur) {
            ...
            $selectsp = "select * from t_param_sp where f_produk_id = '" . $debitur->FacilityType . "' and f_min_dpd <= '" . $debitur->dpd . "' Order By f_sp Desc";
            $hasilselctsp = $this->db2->query($selectsp)->num_rows();
            if ($hasilselctsp > 0) {
                $hasilselctsp2 = $this->db2->query($selectsp)->result();
                foreach ($hasilselctsp2 as $sp) {
                    if ($sp->f_sp == "SP3") {
                        if (((int) $sp->f_min_dpd <= (int) $debitur->dpd)) {
                            ...
                            $selectgeneratesp2 = "select * from t_generate_sp where f_nomer = '" . $nosurat . "'";
                            $selectgeneratesp22 = $this->db2->query($selectgeneratesp2)->num_rows();
                            if ($selectgeneratesp22 > 0) {
                                $hasil[] = "Null";
                            } else {
                                ...
                                if ($hasilselectgeneratesp > 0) {
                                    ...
                                    if ($hasilselectgeneratesp2->f_end_date_time < date('Y-m-d')) {
                                        ...
                                        if ($insertgeneratesp == TRUE) {
                                            $selectsum = "SELECT substr(a.DATEAPPROVED,1,10) as date_aproved ,IFNULL((SELECT d.DESCRIPTION from bss_ld_sub_product as d JOIN bss_cad as e ON d.ID = e.FacilityType WHERE e.ID = a.ID),
										(SELECT w.DESCRIPTION FROM bss_category as w WHERE w.ID = a.FacilityType )) as fasilitas, 
										(select IFNULL(SUM(b.DUE_IN),'0.00') from bss_coll_pd_balaces as b WHERE b.ID = a.id) as denda,
										(select IFNULL(SUM(x.DUE_PR),'0.00') from bss_coll_pd_balaces as x WHERE x.ID = a.id) as pokok,
										(select IFNULL(SUM(v.DUE_CH),'0.00') from bss_coll_pd_balaces as v WHERE v.ID = a.id) as bunga,
										a.Bakidebet as os ,((select IFNULL(SUM(b.DUE_CH),'0.00') from bss_coll_pd_balaces as b WHERE b.ID = a.id) +(select IFNULL(SUM(z.DUE_PR),'0.00') from bss_coll_pd_balaces as z WHERE z.ID = a.id)  + (select ifnull(SUM(y.DUE_IN),'0.00') from bss_coll_pd_balaces as y WHERE y.ID = a.id)) as total_all ,
										a.* FROM bss_cad AS a  WHERE a.NomorNasabah = '" . $debitur->NomorNasabah . "'";
